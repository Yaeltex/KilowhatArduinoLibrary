...
byte inputMapping[32] = {8, 9, 10, 11, 0, 1, 2, 3, 12, 13, 14, 15, 4, 5, 6, 7,
...
void ReadInputs() {
  ...
  for (int inputIndex = 0; inputIndex < numInputs - 4; inputIndex++) {
    ...
    byte inputMapped = inputMapping[inputIndex];
    mux = inputMapped < 16 ? MUX_A : MUX_B;           // MUX 0 or 1
    muxChannel = inputMapped % NUM_MUX_CHANNELS;         // CHANNEL 0-15
    if (inputData.mode() != KMS::M_OFF) {
