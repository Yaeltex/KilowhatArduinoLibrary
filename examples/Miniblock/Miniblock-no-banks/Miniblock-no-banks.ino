...
const byte inputCommonPin = 10, statusLEDPin = 13;
...
bool flagBlinkStatus = 0, midiStatusLED = 0, configMode = 0, receivingSysEx = 0;
...
void blinkStatusLED() {
  ...
  if (millis() - millisPrev > blinkInterval) {
    ...
    digitalWrite(statusLEDPin, !lastLEDState);
  ...
  if (newBank || changeDigOutState) {
    ...
    if (newBank && !configMode) {                                                // If new bank flag is on
      ...
      for (int bankLED = 0; bankLED < MAX_BANKS; bankLED++) {       // and cycle through the LEDs to check which bank is on and which one is off
        if (currBank == bankLED) {                                  // If the current bank matches this LED
          LedWrite(NUM_LEDS - 1 - bankLED, HIGH);                                // turn this LED on.  (7-LED) because they are numbered backwards and LED 0 is on output 7 and so on...
        } else {                                                     // If this LED does not match the current bank selected,
          LedWrite(NUM_LEDS - 1 - bankLED, LOW);                                 // turn this LED off.
        }
      }
    }
    ...
    for (dOut = 0; dOut < NUM_LEDS; dOut++) {                       // Cycle the rest of the LEDs for Miniblock
      if (digitalOutState[currBank][dOut] == OUT_ON)          // If this bank has this LED on, because the bank changed and it was originally on, or because it arrived a matching MIDI message
        LedWrite(NUM_LEDS - 1 - dOut, HIGH);                               // turn this LED on. (3-LED) because they are numbered backwards, LED 0 is on output 3 and so on...
      else if (digitalOutState[currBank][dOut] == OUT_OFF)    // If this LED is meant to be off,
        LedWrite(NUM_LEDS - 1 - dOut, LOW);                                // turn this LED off.
    }
  }
}
  ...
  for (dOut = 0; dOut < NUM_LEDS; dOut++) {                       // Cycle through every LED
    if (digitalOutState[currBank][dOut] == OUT_BLINK) {     // If this output is set to blink
      if (outputBlinkState) {                                 // and it was on
        LedWrite(NUM_LEDS - 1 - dOut, LOW);                                // turn it off.
      }
      else {                                                  // If it was off.
        LedWrite(NUM_LEDS - 1 - dOut, HIGH);                               // turn it on.
      }
    }
  }
...
void HandleNotes() {
  ...
  if (!configMode) {
    for (byte outputIndex = 0; outputIndex < NUM_LEDS; outputIndex++) {     // Scan every output available
      ...
      if ((data1 == outputData.param() || data1 == outputData.param()+4) && channel == outputData.channel()) {   // If channel and note number match an output in the config
        if(data1 == outputData.param()+4){
          outputIndex += 4;
        }
    ...
    if (data2)
      LedWrite(NUM_LEDS - 1 - data1, HIGH);
    else
      LedWrite(NUM_LEDS - 1 - data1, LOW);
  }
}
#endif
...
void pinReadInputs(void) {
  ...
  //Serial.print("BUTTON PRESSED: ");Serial.println(buttonPressed); 
  ...
  for (int buttonPin = 0; buttonPin < 8; buttonPin++) {
    ...
    if (buttonState != buttonPrevState[buttonPin]) {                                                        // Not shifter
      byte buttonPressed = 7 - buttonPin;                         // Button 0 is on pin 3, button 1 is on pin 2, button 2 is on pin 1, button 3 is on pin 0.
      byte numInput = 32 + buttonPressed;                     // Same as Kilowhat
      byte inputIndex;
      if(numInput > 35)
        inputIndex = numInput - 4;
      else
        inputIndex = numInput;

//      Serial.print("Boton: "); Serial.print(buttonPressed);
//      Serial.print(" - "); Serial.println(buttonState);
      ...
      if (inputData.mode() != KMS::M_OFF) {
        ...
        if (buttonState) {    // If the button was pressed
          ...
          InputChanged(numInput, inputData, !buttonStateBanks[currBank][buttonPressed]);
...
void InputChanged(int numInput, const KMS::InputNorm &inputData, uint16_t rawValue) {
  ...
  Serial.print("    Parameter: "); Serial.print(param);
